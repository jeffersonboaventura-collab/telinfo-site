"use client";
import { useState } from "react";

// Números reais da Telinfo
const CONTACTS = {
  br: { number: "5512981279877", label: "Brasil 🇧🇷", display: "+55 12 98127-9877" },
  us: { number: "16893225055",   label: "USA / CA / UK 🇺🇸", display: "+1 689 322 5055" },
};

const MSG = "Olá! Vim pelo site da Telinfo e gostaria de uma avaliação gratuita.";

export default function WhatsAppButton() {
  const [open, setOpen] = useState(false);

  return (
    <div className="fixed bottom-6 right-6 z-[200] flex flex-col items-end gap-2">

      {/* Popup de seleção de número */}
      {open && (
        <div
          className="flex flex-col gap-2 p-3 rounded-xl mb-1"
          style={{
            background: "#0a0f1e",
            border: "1px solid rgba(0,255,136,0.3)",
            boxShadow: "0 8px 32px rgba(0,0,0,0.7), 0 0 16px rgba(0,255,136,0.1)",
            animation: "fade-up 0.2s ease",
          }}
        >
          <p
            className="text-center mb-1"
            style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "rgba(255,255,255,0.4)", letterSpacing: "0.15em" }}
          >
            SELECIONE O PAÍS
          </p>

          {Object.entries(CONTACTS).map(([key, c]) => (
            <a
              key={key}
              href={`https://wa.me/${c.number}?text=${encodeURIComponent(MSG)}`}
              target="_blank"
              rel="noopener noreferrer"
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all duration-200"
              style={{
                background: "rgba(0,255,136,0.05)",
                border: "1px solid rgba(0,255,136,0.15)",
                textDecoration: "none",
              }}
              onMouseEnter={e => (e.currentTarget.style.background = "rgba(0,255,136,0.12)")}
              onMouseLeave={e => (e.currentTarget.style.background = "rgba(0,255,136,0.05)")}
            >
              <svg viewBox="0 0 24 24" fill="#00ff88" className="flex-shrink-0" style={{ width: 18, height: 18 }}>
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              <div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "#00ff88", letterSpacing: "0.1em" }}>{c.label}</div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "#fff", fontWeight: 600 }}>{c.display}</div>
              </div>
            </a>
          ))}
        </div>
      )}

      {/* Botão principal */}
      <button
        onClick={() => setOpen(!open)}
        aria-label="WhatsApp"
        className="flex items-center justify-center w-14 h-14 rounded-full relative transition-all duration-300"
        style={{
          background: "#25d366",
          boxShadow: open
            ? "0 4px 24px rgba(37,211,102,0.6)"
            : "0 4px 20px rgba(37,211,102,0.4)",
          animation: "float 3s ease-in-out infinite",
          transform: open ? "scale(1.1)" : "scale(1)",
        }}
      >
        <svg viewBox="0 0 24 24" fill="white" className="w-7 h-7">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
        {/* Pulse ring */}
        {!open && (
          <span
            className="absolute inset-0 rounded-full border-2 border-green-400 opacity-60"
            style={{ animation: "pulse-neon 2s ease-in-out infinite" }}
          />
        )}
      </button>
    </div>
  );
}




